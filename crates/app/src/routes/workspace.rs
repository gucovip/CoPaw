...
impl IntoResponse for WorkspaceError {
    fn into_response(self) -> Response {
        let (status, message) = match self {
            ...
            WorkspaceError::BadRequest(msg) => (StatusCode::BAD_REQUEST, msg),
...
impl std::fmt::Display for WorkspaceError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            WorkspaceError::BadRequest(msg) => write!(f, "Bad request: {}", msg),
...
pub async fn upload_workspace(_multipart: Multipart) -> Result<Json<UploadSuccessResponse>, WorkspaceError> {
    let working_dir = get_working_dir();

    if !working_dir.is_dir() {
        fs::create_dir_all(&working_dir)
            .await
            .map_err(|e| WorkspaceError::Internal(format!("Failed to create working directory: {}", e)))?;
    }
